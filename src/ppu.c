...
static u32 ppu_dots;
// pixel_info_buffer is array of u32 values containing tile dat in the following format:
// u8 src_msb for 8 pixels | src_lsb for 8 pixels | color_msb for 8 pixels | color_lsb for 8 pixels
u32* pixel_info_buffer;
...
void ppu_init() {
    ...
    pixel_info_buffer = malloc(buffer_size);

    lcd_init();
    memset(bus.oam, 0, sizeof(bus.oam));
    memset(pixel_info_buffer, 0, buffer_size);
}
...
void ppu_mode_vblank() {
    if (ppu_dots >= DOTS_PER_LINE) {
        ...
        if (bus.io.lcd_y >= LINES_PER_FRAME) { 
            ...
            printf("Switching to mode OAM and resetting lcd_y\n");
            bus.io.lcd_y = 0;
        }
...
void fill_ppu_buffer() {
    ...
    u16 start_tile = ((scy+bus.io.lcd_y)/8)*32 + (scx/8);
    u8 tile_row = (scy+bus.io.lcd_y) % 8;
    ...
    for( u16 t = 0; t < 21; t++) { // for each of 21 tiles on scanline that will be on screen
        ...
        u8 tileIndex = mem[tilemapStartAddr + tileAddrAdj];
        
        // get color and src information for tiles from vram
        u8 tilecolors_lsb = mem[vramStartAddr + (tile_row * 2) + (tileIndex * 16)]; // Is this flipped from how display_tile function does this?
        u8 tilecolors_msb = mem[vramStartAddr + (tile_row * 2) + (tileIndex * 16) + 1]; // should tileindex be cast to u16?
        u8 tilesrc_lsb = 0; // TODO: figure out how to get src info
        u8 tilesrc_msb = 0;

        // Combine them into a single 32-bit value
        // pixel_info_buffer is array of u32 values containing tile data in the following format:
        // u8 src_msb for 8 pixels | src_lsb for 8 pixels | color_msb for 8 pixels | color_lsb for 8 pixels
        u32 packed = ((u32)tilesrc_msb << 24) | 
                        ((u32)tilesrc_lsb << 16) |
                        ((u32)tilecolors_msb << 8)  | 
                        ((u32)tilecolors_lsb);        

        // Add info to pixel_info_buffer
        pixel_info_buffer[(u16)bus.io.lcd_y*21 + t] = packed;
