...
void ld_ax() { cpu.reg.a = mem_read(cpu.reg.hl); }  // 0x7E
void ld_an() { cpu.reg.a = fetch(); }  // 0x3E
...
void ld_bx() { cpu.reg.b = mem_read(cpu.reg.hl); }  // 0x46
void ld_bn() { cpu.reg.b = fetch(); }  // 0x06
...
void ld_cx() { cpu.reg.c = mem_read(cpu.reg.hl); }  // 0x4E
void ld_cn() { cpu.reg.c = fetch(); }  // 0x0E
...
void ld_dx() { cpu.reg.d = mem_read(cpu.reg.hl); }  // 0x56
void ld_dn() { cpu.reg.d = fetch(); }  // 0x16
...
void ld_ex() { cpu.reg.e = mem_read(cpu.reg.hl); }  // 0x5E
void ld_en() { cpu.reg.e = fetch(); }  // 0x1E
...
void ld_hx() { cpu.reg.h = mem_read(cpu.reg.hl); }  // 0x66
void ld_hn() { cpu.reg.h = fetch(); }  // 0x26
...
void ld_lx() { cpu.reg.l = mem_read(cpu.reg.hl); }  // 0x6E
void ld_ln() { cpu.reg.l = fetch(); }  // 0x2E

void ld_xb() { mem_write(cpu.reg.hl, cpu.reg.b); }  // 0x70
void ld_xc() { mem_write(cpu.reg.hl, cpu.reg.c); }  // 0x71
void ld_xd() { mem_write(cpu.reg.hl, cpu.reg.d); }  // 0x72
void ld_xe() { mem_write(cpu.reg.hl, cpu.reg.e); }  // 0x73
void ld_xh() { mem_write(cpu.reg.hl, cpu.reg.h); }  // 0x74
void ld_xl() { mem_write(cpu.reg.hl, cpu.reg.l); }  // 0x75
void ld_xa() { mem_write(cpu.reg.hl, cpu.reg.a); }  // 0x77
void ld_xn() { mem_write(cpu.reg.hl, fetch()); }  // 0x36

void ld_a_bc() { cpu.reg.a = mem_read(cpu.reg.bc); }  // 0x0A
void ld_a_de() { cpu.reg.a = mem_read(cpu.reg.de); }  // 0x1A

void ld_bc_a() { mem_write(cpu.reg.bc, cpu.reg.a); }  // 0x02
void ld_de_a() { mem_write(cpu.reg.de, cpu.reg.a); }  // 0x12

void ld_nn_a() { mem_write(fetch16(), cpu.reg.a); }  // 0xEA
void ld_a_nn() { cpu.reg.a = mem_read(fetch16()); }  // 0xFA

void ld_bc_nn() { cpu.reg.bc = fetch16(); }  // 0x01
void ld_de_nn() { cpu.reg.de = fetch16(); }  // 0x11
void ld_hl_nn() { cpu.reg.hl = fetch16(); }  // 0x21
void ld_sp_nn() { cpu.reg.sp = fetch16(); }  // 0x31

void ldio_c_a() { mem_write(0xFF00 + cpu.reg.c, cpu.reg.a); }  // 0xE2
void ldio_a_c() { cpu.reg.a = mem_read(0xFF00 + cpu.reg.c); }  // 0xF2
void ldio_n_a() { mem_write(0xFF00 + fetch(), cpu.reg.a); }  // 0xE0
void ldio_a_n() { cpu.reg.a = mem_read(0xFF00 + fetch()); }  // 0xF0

void ldd_ax() { cpu.reg.a = mem_read(cpu.reg.hl--); }  // 0x3A
void ldd_xa() { mem_write(cpu.reg.hl--, cpu.reg.a); }  // 0x32
void ldi_ax() { cpu.reg.a = mem_read(cpu.reg.hl++); }  // 0x2A
void ldi_xa() { mem_write(cpu.reg.hl++, cpu.reg.a); }  // 0x22
...
void ld_nn_sp() { mem_write16(fetch16(), cpu.reg.sp); }  // 0x08
...
void add_ax() { cpu.reg.a = _add(cpu.reg.a, mem_read(cpu.reg.hl)); }  // 0x86
void add_an() { cpu.reg.a = _add(cpu.reg.a, fetch()); }  // 0xC6
...
void adc_ax() { cpu.reg.a = _adc(cpu.reg.a, mem_read(cpu.reg.hl)); }  // 0x8E
void adc_an() { cpu.reg.a = _adc(cpu.reg.a, fetch()); }  // 0xCE
...
void sub_ax() { cpu.reg.a = _sub(cpu.reg.a, mem_read(cpu.reg.hl)); }  // 0x96
void sub_an() { cpu.reg.a = _sub(cpu.reg.a, fetch()); }  // 0xD6
...
void sbc_ax() { cpu.reg.a = _sbc(cpu.reg.a, mem_read(cpu.reg.hl)); }  // 0x9E
void sbc_an() { cpu.reg.a = _sbc(cpu.reg.a, fetch()); }  // 0xDE
...
void and_ax() { cpu.reg.a = _and(cpu.reg.a, mem_read(cpu.reg.hl)); }  // 0xA6
void and_an() { cpu.reg.a = _and(cpu.reg.a, fetch()); }  // 0xE6
...
void xor_ax() { cpu.reg.a = _xor(cpu.reg.a, mem_read(cpu.reg.hl)); }  // 0xAE
void xor_an() { cpu.reg.a = _xor(cpu.reg.a, fetch()); }  // 0xEE
...
void or_ax() { cpu.reg.a = _or(cpu.reg.a, mem_read(cpu.reg.hl)); }  // 0xB6
void or_an() { cpu.reg.a = _or(cpu.reg.a, fetch()); }  // 0xF6
...
void cmp_ax() { _cmp(cpu.reg.a, mem_read(cpu.reg.hl)); }  // 0xBE
void cmp_an() { _cmp(cpu.reg.a, fetch()); }  // 0xFE
...
void inc_x() { mem_write(cpu.reg.hl, _inc(mem_read(cpu.reg.hl))); }  // 0x34
...
void dec_x() { mem_write(cpu.reg.hl, _dec(mem_read(cpu.reg.hl))); }  // 0x35
...
void jmp_z() { u16 nn = fetch16(); if (cpu.flag.z) cpu_jump(nn); }  // 0xCA
void jmp_c() { u16 nn = fetch16(); if (cpu.flag.c) cpu_jump(nn); }  // 0xDA
void jmp_nz() { u16 nn = fetch16(); if (!cpu.flag.z) cpu_jump(nn); }  // 0xC2
void jmp_nc() { u16 nn = fetch16(); if (!cpu.flag.c) cpu_jump(nn); }  // 0xD2

void jmpr() { i8 n = fetch(); cpu_jmpr(n); }  // 0x18
void jmpr_z() { i8 n = fetch(); if (cpu.flag.z) cpu_jmpr(n); }  // 0x28
void jmpr_c() { i8 n = fetch(); if (cpu.flag.c) cpu_jmpr(n); }  // 0x38
void jmpr_nz() { i8 n = fetch(); if (!cpu.flag.z) cpu_jmpr(n); }  // 0x20
void jmpr_nc() { i8 n = fetch(); if (!cpu.flag.c) cpu_jmpr(n); }  // 0x30
...
void call() { u16 nn = fetch16(); stack_push16(cpu.reg.pc); cpu_jump(nn); }  // 0xCD
void call_z() { u16 nn = fetch16(); if (cpu.flag.z) { stack_push16(cpu.reg.pc); cpu_jump(nn); } }  // 0xCC
void call_c() { u16 nn = fetch16(); if (cpu.flag.c) { stack_push16(cpu.reg.pc); cpu_jump(nn); } }  // 0xDC
void call_nz() { u16 nn = fetch16(); if (!cpu.flag.z) { stack_push16(cpu.reg.pc); cpu_jump(nn); } }  // 0xC4
void call_nc() { u16 nn = fetch16(); if (!cpu.flag.c) { stack_push16(cpu.reg.pc); cpu_jump(nn); } }  // 0xD4
...
void rlc_x() { mem_write(cpu.reg.hl, _rlc(mem_read(cpu.reg.hl))); }  // CB 06
...
void rrc_x() { mem_write(cpu.reg.hl, _rrc(mem_read(cpu.reg.hl))); }  // CB 0E
...
void rl_x() { mem_write(cpu.reg.hl, _rl(mem_read(cpu.reg.hl))); }  // CB 16
...
void rr_x() { mem_write(cpu.reg.hl, _rr(mem_read(cpu.reg.hl))); }  // CB 1E
...
void sla_x() { mem_write(cpu.reg.hl, _sla(mem_read(cpu.reg.hl))); }  // CB 26
...
void sra_x() { mem_write(cpu.reg.hl, _sra(mem_read(cpu.reg.hl))); }  // CB 2E
...
void swap_x() { mem_write(cpu.reg.hl, _swap(mem_read(cpu.reg.hl))); }  // CB 36
...
void srl_x() { mem_write(cpu.reg.hl, _srl(mem_read(cpu.reg.hl))); }  // CB 3E
...
void bit0_x() { _bit(mem_read(cpu.reg.hl), 0); }  // CB 46
...
void bit1_x() { _bit(mem_read(cpu.reg.hl), 1); }  // CB 4E
...
void bit2_x() { _bit(mem_read(cpu.reg.hl), 2); }  // CB 56
...
void bit3_x() { _bit(mem_read(cpu.reg.hl), 3); }  // CB 5E
...
void bit4_x() { _bit(mem_read(cpu.reg.hl), 4); }  // CB 66
...
void bit5_x() { _bit(mem_read(cpu.reg.hl), 5); }  // CB 6E
...
void bit6_x() { _bit(mem_read(cpu.reg.hl), 6); }  // CB 76
...
void bit7_x() { _bit(mem_read(cpu.reg.hl), 7); }  // CB 7E
...
void res0_x() { mem_write(cpu.reg.hl, bit_clear(mem_read(cpu.reg.hl), 0)); }  // CB 86
...
void res1_x() { mem_write(cpu.reg.hl, bit_clear(mem_read(cpu.reg.hl), 1)); }  // CB 8E
...
void res2_x() { mem_write(cpu.reg.hl, bit_clear(mem_read(cpu.reg.hl), 2)); }  // CB 96
...
void res3_x() { mem_write(cpu.reg.hl, bit_clear(mem_read(cpu.reg.hl), 3)); }  // CB 9E
...
void res4_x() { mem_write(cpu.reg.hl, bit_clear(mem_read(cpu.reg.hl), 4)); }  // CB A6
...
void res5_x() { mem_write(cpu.reg.hl, bit_clear(mem_read(cpu.reg.hl), 5)); }  // CB AE
...
void res6_x() { mem_write(cpu.reg.hl, bit_clear(mem_read(cpu.reg.hl), 6)); }  // CB B6
...
void res7_x() { mem_write(cpu.reg.hl, bit_clear(mem_read(cpu.reg.hl), 7)); }  // CB BE
...
void set0_x() { mem_write(cpu.reg.hl, bit_set(mem_read(cpu.reg.hl), 0)); }  // CB C6
...
void set1_x() { mem_write(cpu.reg.hl, bit_set(mem_read(cpu.reg.hl), 1)); }  // CB CE
...
void set2_x() { mem_write(cpu.reg.hl, bit_set(mem_read(cpu.reg.hl), 2)); }  // CB D6
...
void set3_x() { mem_write(cpu.reg.hl, bit_set(mem_read(cpu.reg.hl), 3)); }  // CB DE
...
void set4_x() { mem_write(cpu.reg.hl, bit_set(mem_read(cpu.reg.hl), 4)); }  // CB E6
...
void set5_x() { mem_write(cpu.reg.hl, bit_set(mem_read(cpu.reg.hl), 5)); }  // CB EE
...
void set6_x() { mem_write(cpu.reg.hl, bit_set(mem_read(cpu.reg.hl), 6)); }  // CB F6
...
void set7_x() { mem_write(cpu.reg.hl, bit_set(mem_read(cpu.reg.hl), 7)); }  // CB FE
